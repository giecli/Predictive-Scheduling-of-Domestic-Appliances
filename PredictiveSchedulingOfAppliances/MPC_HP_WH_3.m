function [out1, out2, out3, out4] = MPC_HP_WH_3(x0,Ad,Bd,Cd,Ed,nu,ymax,ymin,cost,weather,x0_w,Adw,Bdw,Cdw,Edw,nu_w,ymax_w,ymin_w,dist,capacity,N,T_run_3,total_I3_0,deadline_sim_3,umax,umin,u_w_max,u_w_min,dumax,dumin,du_w_max,du_w_min,t)
objective = 0;
constraints = [];
%heat pump
...
for k = 1 : N
    ...
    u_3{k} = 3600*3*I3{k};
    %capacity constraint
    constraints = [constraints, I1{k} + I2{k} + u_3{k} <= capacity(k) + v_capacity{k}];
    constraints = [constraints, v_capacity{k} >= 0];
...
%     constraints = [constraints, implies(k <= deadline_sim_3-t, d)];
%     constraints = [constraints, implies(d, total_I3{deadline_sim_3 - t} == T_run_3)];

end
ops = sdpsettings('solver','gurobi','gurobi.mipgapabs', 0.1, 'gurobi.timelimit', 50,'verbose', 0);
