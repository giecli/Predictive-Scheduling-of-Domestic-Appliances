function [out1, out2, out3, out4] = MPC_HP_WH_4(x0,Ad,Bd,Cd,Ed,nu,ymax,ymin,cost,weather,x0_w,Adw,Bdw,Cdw,Edw,nu_w,ymax_w,ymin_w,dist,capacity,N,T_run_4,total_I4_0,deadline_sim_4,umax,umin,u_w_max,u_w_min,dumax,dumin,du_w_max,du_w_min,t)
objective = 0;
constraints = [];
%heat pump
...
for k = 1 : N
    ...
    u_4{k} = 3600*4*I4{k};
    %capacity constraint
    constraints = [constraints, I1{k} + I2{k} + u_4{k} <= capacity(k) + v_capacity{k}];
...
%     constraints = [constraints, implies(k <= deadline_sim_4-t, d)];
%     constraints = [constraints, implies(d, total_I4{deadline_sim_4 - t} == T_run_4)];    
